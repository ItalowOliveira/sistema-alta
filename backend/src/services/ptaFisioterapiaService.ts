import { createPtaFisioterapiaRepository } from '../repositories/ptaFisioterapiaRepository';
...
import { getPtaFisioterapiaByIdRepository } from '../repositories/ptaFisioterapiaRepository';

export const createPtaFisioterapiaService = async (data: any) => {
  ...
  const created = await createPtaFisioterapiaRepository(payload);
  ...
  if (altaNum !== null) {
    try {
      ...
      try {
        ...
        if (newId) {
          await updateAltaDocumentoRepository(altaNum, 'PTA_FISIOTERAPIA', Number(newId));
        }
      } catch (errDoc) {
        console.error('Erro atualizando documento da alta (PTA Fisioterapia):', errDoc);
      }
      ...
      if (data && (data as any).data_alta) {
        try {
          ...
          console.error('Erro atualizando data_alta da alta (PTA Fisioterapia):', errData);
        }
      }
    } catch (err) {
      console.error('Erro atualizando status da alta (PTA Fisioterapia):', err);
    }
  }
...
export const getPtaFisioterapiaByIdService = async (id: number) => {
  if (!id || Number.isNaN(Number(id)) || id <= 0) return null;
  const pta = await getPtaFisioterapiaByIdRepository(Number(id));
  return pta;
};
